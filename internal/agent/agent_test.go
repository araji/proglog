...
import (
	...
	"github.com/araji/proglog/internal/agent"
...
func TestAgent(t *testing.T) {
	...
	// wait until the leader has replicated the record to the follower
	time.Sleep(3 * time.Second)
...
func client(
	...
	rpcAdrr, err := agent.Config.RPCAddr()
	require.NoError(t, err)
	conn, err := grpc.Dial(rpcAdrr, opts...)
	require.NoError(t, err)
